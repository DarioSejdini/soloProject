...
class User:
    ...
    @classmethod
    def get_user_liked_pies(cls, data):
        query = "SELECT votes.pies_id as id, pies.name as name  from votes LEFT JOIN pies on votes.pies_id = pies.id WHERE votes.users_id = %(user_id)s;"
        results = connectToMySQL(cls.db_name).query_db(query, data)
        likedPies = []
        if results:
            for row in results:
                likedPies.append(row)
            return likedPies
        return likedPies
    
    @classmethod
    def get_user_liked_piesId(cls, data):
        query = "SELECT votes.pies_id as id, pies.name as name  from votes LEFT JOIN pies on votes.pies_id = pies.id WHERE votes.users_id = %(user_id)s;"
        results = connectToMySQL(cls.db_name).query_db(query, data)
        likedPies = []
        if results:
            for row in results:
                likedPies.append(row['id'])
            return likedPies
        return likedPies
